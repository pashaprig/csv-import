...
Підтримуються два формати вводу:

1. Повний опис маршруту:
Маршрут 1
Завдання: патрулювання району
Маршрут: 50.4501, 30.5234 -> 50.4547, 30.5167 -> 50.4488, 30.5245

У повному форматі:
- рядок "Маршрут ..." використовується як назва;
- текст після "Завдання:" записується в додаткову інформацію;
- текст після "Маршрут:" містить точки маршруту в порядку проходження.

2. Лише координати, по одній точці в кожному рядку:
50.4501, 30.5234
50.4547, 30.5167
50.4488, 30.5245

Для одного маршруту потрібно вказати щонайменше дві точки. Точки у повному форматі можна розділяти символами "->", "→", "➝", "⟶" або "↦". Програма нормалізує їх і збереже точки в одному полі coordinates.

Щоб додати кілька маршрутів у повному форматі, починайте кожен блок із нового рядка "Маршрут ..." і відокремлюйте блоки порожнім рядком. Після натискання "Підготувати таблицю" кожен блок стане окремим рядком із геометрією LineString.
</div>
</section>

<section class="csv__modal-section" id="parsing-polygons">
  <h3 class="csv__modal-section-title">Полігони</h3>
  <div class="csv__modal-copy">Режим підготовки полігонів буде додано окремо.

У цьому режимі будуть задаватися координати замкненого контуру об'єкта. Після активації режиму тут з'явиться точний формат введення та приклади.</div>
</section>`,
      confirmText: "Закрити",
      singleButton: true,
      onConfirm: () => {}
